...
export default class MemoryBucket {
    ...
    async head(key) {
        ...
        return object ? { key, size: object.bytes.byteLength, uploaded: object.uploaded, httpMetadata: object.httpMetadata } : null;
    }
    ...
    async put(key, value, options = {}) {
        let bytes = value instanceof ReadableStream
    ...
    async list({ prefix = '', cursor, limit = 1000 } = {}) {
        ...
        return {
            objects: page.map(key => ({ key, size: this.objects.get(key).bytes.byteLength, uploaded: this.objects.get(key).uploaded })),
            truncated,
