...
const data = {
  regular: [
    {
      stage_a: { id: '0', name: 'The Reef' },
      stage_b: { id: '22', name: 'Skipper Pavilion' },
...
test('records localized values under name/ids/value and reports what is missing', async () => {
  ...
  assert.equal(
    (await bucket.head('data/locale/en.json')).httpMetadata.cacheControl,
...
test('supports multi-part ids and creates a missing document', async () => {
  ...
  await processor.updateLocalizations({
    merchandises: [{ gear: { kind: 'head', id: '5', name: 'Fake Hat' } }],
  });

  assert.deepEqual(await json(bucket, 'data/locale/en.json'), {
    gear: { head: { 5: { name: 'Fake Hat' } } },
  });
});
...
test('does not rewrite a document that already has every string', async () => {
  ...
  await bucket.put(
    'data/locale/en.json',
    JSON.stringify({ stages: { 0: { name: 'The Reef' }, 22: { name: 'Skipper Pavilion' } } }),
  );
