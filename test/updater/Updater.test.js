...
function thingsFor(language) {
  ...
  return {
    things: [
      { id: '1', name: names.thing, image: '/images/things/one.png' },
      { id: '2', name: names.other, image: '/images/things/two.png' },
    ],
  };
}
...
test('publishes data, locale document, and calendar with the right metadata', async () => {
  ...
  assert.deepEqual(await json(b.publicBucket, 'data/locale/en.json'), {
    things: { 1: { name: 'Widget' }, 2: { name: 'Gadget' } },
  });
  assert.deepEqual(await json(b.publicBucket, 'data/locale/ja.json'), {
    things: { 1: { name: 'ウィジェット' }, 2: { name: 'ガジェット' } },
  });
...
test('fetches each language with missing strings using that region\'s session, then stops needing to', async () => {
  ...
  assert.deepEqual(
    splatnet.api.map(r => `${r.region}/${r.language}`),
  ...
  assert.deepEqual(
    second.api.map(r => r.language),
    ['en'],
  );
  assert.deepEqual(
    puts.mock.calls.map(c => c.arguments[0]).filter(k => k.startsWith('data/locale/')),
    [],
  );
});
...
test('mirrors only images that are missing, using one list per directory', async () => {
  ...
  assert.deepEqual(
    splatnet.images.map(r => r.path),
    ['/images/things/two.png'],
  );
  ...
  assert.deepEqual(
    list.mock.calls.map(c => c.arguments[0].prefix),
    ['assets/splatnet/images/things/'],
  );
  assert.equal(head.mock.calls.length, 0);
  assert.equal(
    (await b.publicBucket.get('assets/splatnet/images/things/two.png')).httpMetadata.contentType,
    'image/png',
  );
});
...
test('uses the embedded backup for the image SplatNet no longer serves', async () => {
  ...
  const splatnet = fakeSplatNet({
    '/api/things': () => ({ things: [{ id: '1', name: 'Widget', image: missing }] }),
  });
...
test('filters root keys, dropping hidden timeline items', async () => {
  fakeSplatNet({
    '/api/things': () => ({ keep: { importance: 1, x: 1 }, hidden: { importance: -1 }, extra: 'dropped' }),
  });
  await new ThingsUpdater(b, {
    rootKeys: ['keep', 'hidden', 'absent'],
    localization: undefined,
    imagePaths: undefined,
    calendarFilename: undefined,
  }).update();

  assert.deepEqual(await json(b.publicBucket, 'data/things.json'), {
    keep: { importance: 1, x: 1 },
    hidden: null,
    absent: null,
  });
});
