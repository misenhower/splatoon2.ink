...
const rotation = (start, a, b, rule) => ({
  start_time: start,
  end_time: start + 7200,
  stage_a: a,
  stage_b: b,
  rule: { key: rule, name: rule },
});
...
test('createPosts builds every post against the same storage and clients', () => {
  ...
  assert.deepEqual(
    posts.map(t => t.getKey()),
    ['schedule', 'gear', 'salmonrun', 'weapon', 'newstage', 'splatfest-na', 'splatfest-eu', 'splatfest-jp'],
  );
  assert.ok(
    posts.every(
      t => t.publicStorage === s.publicStorage && t.clients[0] === client && t.screenshots === screenshots,
    ),
  );
});
...
test('schedule: posts the rotation for the current hour, with new-stage wording when the stage is new', async () => {
  ...
  await seed(s.publicBucket, 'data/schedules.json', {
    regular: [rotation(NOW, reef, skipper, 'Turf War')],
    gachi: [rotation(NOW, reef, skipper, 'Splat Zones')],
    league: [rotation(NOW, reef, skipper, 'Rainmaker')],
  });
  await seed(s.privateBucket, 'stages.json', [
    { ...reef, first_seen: -1, first_available: -1 },
    { ...skipper, first_seen: NOW - 7200, first_available: NOW },
  ]);
...
test('schedule: the plain rotation text names the ranked and league rules', async () => {
  ...
  await seed(s.publicBucket, 'data/schedules.json', {
    regular: [rotation(NOW, reef, fitness, 'Turf War')],
    gachi: [rotation(NOW, reef, fitness, 'Splat Zones')],
    league: [rotation(NOW, reef, fitness, 'Rainmaker')],
  });

  const post = pinTime(new SchedulePost(s, [client]), NOW);

  assert.equal(
    await post.getText(await post.getData()),
    'Splatoon 2 map rotation: Ranked game mode: Splat Zones, League game mode: Rainmaker #maprotation',
  );
});
...
test('salmon run: posts when a shift opens, remembers it, and posts again when it closes', async () => {
  const shift = {
    start_time: NOW,
    end_time: NOW + 3600 * 36,
    stage: { name: 'Spawning Grounds' },
    weapons: [{ id: '0', weapon: { name: 'Splattershot' } }],
  };
  ...
  assert.equal(
    client.sent[0].status,
    'Salmon Run is now open on Spawning Grounds! Current reward gear is the Cap. #salmonrun #splatoon2',
  );
  ...
  assert.equal(
    client.sent[2].status,
    'Salmon Run is now closed. The next shift starts in 4 hours! #salmonrun #splatoon2',
  );
});
...
test('new stage: posts the mirrored stage image the hour the stage was first seen', async () => {
  await seed(s.privateBucket, 'stages.json', [
    {
      ...
      image: '/images/stage/22.png',
      first_seen: NOW,
      first_available: NOW + 7200,
    },
  ]);
  ...
  assert.equal(
    client.sent[0].status,
    'NEW STAGE: The first schedules for Skipper Pavilion have been posted! Start playing the new stage when this post is 2 hours old. #splatoon2',
  );
...
test('splatfest: a global fest posts once, from the first region', async () => {
  const fest = {
    ...
    times: { announce: 0, start: NOW, end: NOW + 3600, result: NOW + 7200 },
  };

  await seed(s.publicBucket, 'data/festivals.json', {
    na: { festivals: [fest], results: [] },
    eu: { festivals: [fest], results: [] },
    jp: { festivals: [fest], results: [] },
  });
  ...
  for (const region of ['na', 'eu', 'jp']) {
    ...
    if (c.sent.length)
      sentBy.push([region, c.sent[0].status]);
  }
