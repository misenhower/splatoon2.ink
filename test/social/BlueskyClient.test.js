...
function fakeAgent() {
  ...
  return {
    ...
    uploadBlob: async (bytes, options) => {
      calls.uploads.push({ bytes, options });

      return { data: { blob: { ref: `blob-${calls.uploads.length}` } } };
    },
    post: async record => {
...
test('uploads the media as given and posts with an image embed and aspect ratio', async () => {
  ...
  await client.send({
    status: 'Hello #splatoon2',
    media: [{ file: jpeg, type: 'image/jpeg', width: 2432, height: 1368 }],
  });
  ...
  assert.deepEqual(post.embed, {
    $type: 'app.bsky.embed.images',
    images: [{ image: { ref: 'blob-1' }, alt: '', aspectRatio: { width: 2432, height: 1368 } }],
  });
});
...
test('omits the aspect ratio when the size is unknown', async () => {
  ...
  await new BlueskyClient({ agent }).send({
    status: 'x',
    media: [{ file: new Uint8Array([1]), type: 'image/png' }],
  });
...
test('cannot send without credentials', async () => {
  for (const name of ['BLUESKY_SERVICE', 'BLUESKY_IDENTIFIER', 'BLUESKY_PASSWORD'])
