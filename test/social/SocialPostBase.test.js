...
test('captures one PNG and converts it once for JPEG clients, preserving dimensions', async () => {
  const png = await sharp({ create: { width: 2, height: 1, channels: 3, background: '#ff0000' } })
    .png()
    .toBuffer();
  ...
  const secondJpeg = fakeClient('second');
  secondJpeg.mediaType = 'image/jpeg';
...
test('test screenshots go to public storage', async () => {
  ...
  assert.deepEqual(
    new Uint8Array(await (await s.publicBucket.get('test-screenshots/hourly.png')).arrayBuffer()),
    IMAGE,
  );
});
