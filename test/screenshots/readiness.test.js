...
test('waits for data, Vue, fonts, images and two layout frames before signaling readiness', async () => {
    const data = deferred(), vue = deferred(), fonts = deferred(), image = deferred();
    const steps = [];
    const document = {
        documentElement: { removeAttribute: () => steps.push('reset'), setAttribute: (name, value) => steps.push([name, value]) },
        fonts: { ready: fonts.promise },
        images: [{ decode: () => { steps.push('image'); return image.promise; } }],
    };
    const ready = markScreenshotReady({
        document, loadData: () => data.promise,
        nextTick: () => { steps.push('vue'); return vue.promise; },
        requestAnimationFrame: callback => { steps.push('frame'); callback(); },
    });
...
test('failed data and superseded routes never mark a page ready', async () => {
    let marked = false;
    const document = {
        documentElement: { removeAttribute() {}, setAttribute() { marked = true; } },
        fonts: { ready: Promise.resolve() }, images: [],
    };
    const options = { document, nextTick: async () => {}, requestAnimationFrame: callback => callback() };
    await assert.rejects(markScreenshotReady({ ...options, loadData: async () => [{ status: 'rejected' }] }), /data failed/);
