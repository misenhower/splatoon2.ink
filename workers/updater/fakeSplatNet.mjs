...
const rotation = start => ({
  ...
  start_time: start,
  end_time: start + 7200,
  stage_a: stage('0', 'The Reef'),
  stage_b: stage('1', 'Musselforge Fitness'),
...
const festival = id => ({
  ...
  special_stage: stage('100', 'Shifty Station'),
});
...
export const ROUTES = {
  ...
  '/api/coop_schedules': () => ({
    schedules: [{ start_time: 3600, end_time: 7200 }],
    details: [
      {
        start_time: 3600,
        end_time: 7200,
  ...
  '/api/timeline': () => ({
    coop: { importance: 1, reward_gear: { available_time: 0, gear } },
    weapon_availability: null,
  }),
  ...
  '/api/onlineshop/merchandises': () => ({
    merchandises: [{ end_time: 7200, gear, skill: { id: '0', name: 'Skill', image: '/images/skill/0.png' } }],
  }),
};
