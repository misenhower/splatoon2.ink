...
describe('runUpdaters', () => {
  ...
  it('runs every updater against the R2 bindings and publishes the site data', async () => {
    ...
    expect(await keys(env.ASSETS, 'data/')).toEqual(
      expect.arrayContaining([
        'data/schedules.json',
        'data/coop-schedules.json',
        'data/coop-schedules.ics',
        'data/timeline.json',
        'data/festivals.json',
        'data/festivals-na.ics',
        'data/festivals-eu.ics',
        'data/festivals-jp.ics',
        'data/festivals/na-1-rankings.json',
    ...
    expect((await env.ASSETS.head('data/schedules.json')).httpMetadata.cacheControl).toContain(
      'stale-while-revalidate',
    );
    expect(await keys(env.ASSETS, 'assets/splatnet/images/stage/')).toEqual([
      'assets/splatnet/images/stage/0.png',
      'assets/splatnet/images/stage/1.png',
      'assets/splatnet/images/stage/100.png',
    ]);
  ...
  it('can run a subset of updaters, and reports a failing one without stopping the rest', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      fakeSplatNet({
        '/api/timeline': () => new Response('down', { status: 503 }),
        '/api/onlineshop/merchandises': () => ({ merchandises: [] }),
      }),
    );
